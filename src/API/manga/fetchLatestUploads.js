...
function groupChaptersByMangaID(chapters) {
  ...
  chapters.forEach((chapter) => {
    ...
    const existingGroup = groupedChapters.find(
      (group) => group.mangaID === mangaID
    );
    if (existingGroup) {
...
export default async function fetchLatestUploads(
  ...
  try {
    ...
    const response = await axios.get(
      `${process.env.REACT_APP_PROXY_SERVER_URL}/api/v1/chapter`,
      {
        params: params,
      }
    );
    ...
    const chaptersGroupedByMangaID = groupChaptersByMangaID(data);
