...
const MangaDetailsSection = ({ id, mangaData, statistics, isStatsLoading }) => {
                ...
                !isStatsLoading && statistics ?
                    <div className='flex flex-wrap justify-center my-4 text-center xl:flex-nowrap gap-y-1'>
                        {mangaDetails.map((detail, index) => (
                            <div
                                key={index}
                                className='text-[10px] md:text-[15px] px-4 md:px-7 py-2 basis-1/3 border-x-[1.5px] border-opacity-30 md:mx-[2px] border-[#1F1F1F] font-medium tracking-[0.1em] lg:tracking-[0.2em] cursor-pointer bg-white text-[#1F1F1F]'
                            >
                                {
