        ...
        <>
            {
                isCoverError || ischaptersError ?
                    ...
                    :
                    <div
                        className="w-full flex-shrink-0 relative flex justify-left pt-[110px] lg:pt-[100px] pl-2 md:pl-4 pr-4 lg:pr-0 lg:pl-[70px] xl:pl-[120px] bg-gray-600"
                    >
                        <img decoding='async' fetchPriority='high' className='absolute w-full bg-cover md:bg-contain md:w-[300px] h-full z-10 inset-0 left-1/2 transform -translate-x-1/2' loading='eager' src={`${!isCoverLoading && `${PROXY_SERVER_URL}/images/${manga.id}/${encodeURIComponent(imageUrl)}`}`} alt='' />
                        <img decoding='async' fetchPriority='high' className="hidden md:block absolute w-full inset-0 bg-[#1F1F1F] bg-cover bg-center opacity-20 rounded-lg" loading='eager' src={`${!isCoverLoading && `${PROXY_SERVER_URL}/images/${manga.id}/${encodeURIComponent(imageUrl)}`}`} alt='' />
                        <div className='absolute inset-0 z-20 w-full bg-black bg-center bg-cover rounded-lg opacity-40' />
                        <div className="absolute z-30 flex items-center px-2 py-1 font-semibold tracking-[0.1em] bg-white text-gray-800 space-x-2 rounded-md top-4 right-4">
                        ...
                        {
                            isChaptersLoading ?
                                <div className="absolute bottom-4 left-4 z-30 font-semibold tracking-[0.4em] bg-white text-gray-800 px-10 py-2 rounded-lg">
                                    Loading
                                </div>
                                :
                                sortedChapters === null ? (
                                    <div className="absolute bottom-4 left-2 z-30 md:left-4 text-[13px] md:text-[16px] font-semibold tracking-[0.4em] bg-white text-gray-800 px-10 py-2 rounded-lg">
                                        No Chapter
                                    </div>
                                ) : (
                                    <div className="absolute bottom-4 left-2 z-30 md:left-4 font-semibold tracking-[0.4em] bg-white text-gray-800 px-10 py-2 rounded-lg">
