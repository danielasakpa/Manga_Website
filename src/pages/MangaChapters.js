        ...
        <div className="text-white">
            {
                chaptersData.data.length < 1 ?
                    ...
                    :
                    <div className="overflow-x-hidden overflow-auto h-[500px] mt-16 px-10">
                        {sortedChapters.map((chapter) => (
                            <Link key={chapter.id} className='' to={`/manga/${id}/chapter/${chapter.id}`}>
                                <div className="py-4 px-3 bg-white my-3 text-black rounded-md font-medium hover:bg-[#E40066] hover:text-white cursor-pointer" key={chapter.id}>
